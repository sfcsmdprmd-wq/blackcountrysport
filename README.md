...
Version 1.4 adds a wall-clock PCM jitter buffer between the SignalFlow decoder and the Icecast encoder. This prevents bursty decoder output, FFmpeg scheduling jitter, or short source stalls from being written directly to Icecast as audible glitches. It also logs buffer underruns and Icecast backpressure every 30 seconds so the remaining fault can be identified precisely. Version 1.4 retains default-source looping and extro overlap mixing. If upgrading an existing Railway or Docker deployment, replace the package files, rebuild the service, and keep the existing environment variables unchanged.
...
Version 1.4 writes an `Audio health` line every 30 seconds, for example:

```text
Audio health: buffer=180ms underruns=0 backpressure=0 dropped=0ms
```

- `underruns=0` is ideal. Repeated underruns mean the scheduled source or decoder cannot provide PCM consistently.
- `backpressure=0` is ideal. Repeated backpressure with no underruns points toward the Icecast/source connection or encoder output path.
- `dropped=0ms` is ideal. Dropped audio means the decoder is running materially ahead of real time and the guard buffer had to discard stale PCM to keep clock time aligned.
