...
Version 1.4 or later is required for reliable uploaded-file handovers. It trims late-joined WAV, MP3 and AAC uploads after decoding instead of asking the HTTP endpoint to seek into the file, preventing `could not seek`, empty-output and corrupt-packet failures during back-timed and fixed-time transitions. It also isolates replaced source pipelines and honours Icecast output backpressure. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
...
- **`could not seek` or empty output during a handover:** confirm the deployed package reports version 1.4.0 or later, then force a clean rebuild so Railway or Docker is not running a cached encoder file.
