...
Version 1.7 or later is required for accurate scheduled starts, reliable continuous output and uploaded-file handovers. The encoder prepares each handover three seconds early, holds the incoming item until its exact clock time, then starts it at 0:00 while the outgoing source fades over it. That mixer remains active after the fade instead of restarting the decoder during the opening. Icecast remains the single real-time clock, the silence fallback waits for a genuine outage, and uploaded files are cached locally. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
