...
Version 2.0 uses warm audio-pipeline handovers. The current pipeline continues
feeding Icecast until its replacement has decoded valid PCM, preventing gaps
and stutters when a default stream, an extro-enabled audio item, and the next
scheduled stream overlap. Mixed pipelines also remain active for the complete
outgoing audio tail instead of being limited by the incoming item's end time.

This service follows the SignalFlow playout schedule and maintains one continuous MP3 or AAC source connection to an Icecast-compatible server. It plays silence when the schedule is empty, switches automatically between scheduled items, seeks into late-joined audio files, mixes extro overlaps, and fades outgoing audio beneath the next item.

Version 1.9 or later is required for accurate scheduled starts, complete audio tails, reliable continuous output and uploaded-file handovers. When an uploaded file has an extro, the next item starts at that point while the entire remaining portion of the file fades underneath until its natural end; it is no longer limited to a three-second tail. Streams and default sources retain a controlled three-second fade. Cached uploads are passed to FFmpeg as local filesystem paths, and each handover is prepared in advance without restarting the decoder during the opening. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
...
- **`could not seek`, `Cannot determine format` or empty output during a handover:** confirm the deployed package reports version 1.5.0 or later, then force a clean rebuild so Railway or Docker is not running a cached encoder file.
