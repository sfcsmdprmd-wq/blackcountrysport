...
Version 1.6 or later is required for reliable continuous output and uploaded-file handovers. Icecast is the single real-time clock: source decoders may buffer ahead but pause cleanly when the output is full. The silence fallback now waits for a genuine two-second source outage and never writes while the encoder is applying backpressure, preventing alternating bursts of programme audio and silence. Uploaded WAV, MP3 and AAC files are cached locally, and a five-second opening-protection window preserves the beginning of scheduled files during fades. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
