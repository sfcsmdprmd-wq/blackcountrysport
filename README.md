...
Version 1.5 or later is required for reliable uploaded-file handovers. It downloads upcoming WAV, MP3 and AAC uploads into a local cache and reuses the stable copy during transitions, preventing repeated HTTP reads from ending before FFmpeg has identified the file. A five-second opening-protection window ensures a newly started scheduled file begins at 0:00 even when FFmpeg needs a moment to establish the fade; later decoder restarts and genuinely late joins still resume at the correct position. It also enlarges every input queue, isolates replaced source pipelines and honours Icecast output backpressure. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
