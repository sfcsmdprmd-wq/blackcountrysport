...
Version 1.2 or later is required for stable extro mixing. It uses FFmpeg's native input buffering and audio mixer so network timing cannot create gaps in the Icecast output. If upgrading an existing Railway or Docker deployment, replace `encoder.mjs` and `package.json`, rebuild the service, and keep the existing environment variables unchanged.
