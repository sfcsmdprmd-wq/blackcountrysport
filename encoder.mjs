...
function startDecoder(status){
  ...
  for(const input of inputs){let url=new URL(input.playbackUrl,site).toString(),remote=input.item.kind==="stream"&&input.item.sourceUrl?new URL(input.item.sourceUrl):null,useOverride=!!remote&&!!overrideHost&&!!overrideIp&&remote.hostname===overrideHost&&!override;if(useOverride){url="pipe:0";override=remote}args.push("-re");if(url.startsWith(site)&&ffmpegHeader)args.push("-headers",ffmpegHeader);const rawElapsed=Math.max(0,(now-new Date(input.item.startAt).getTime())/1000),elapsed=input.item.loop&&input.item.durationSeconds?rawElapsed%input.item.durationSeconds:rawElapsed;if(input.item.loop)args.push("-stream_loop","-1");if(input.item.kind==="upload"&&elapsed>0)args.push("-ss",elapsed.toFixed(3));args.push("-i",url)}
  const currentRemaining=Math.max(.1,(new Date(status.current.endAt).getTime()-now)/1000);
  if(inputs.length>1){const filters=[],labels=[];inputs.forEach((input,index)=>{if(input.outgoing){const remaining=Math.max(.1,(new Date(input.item.endAt).getTime()-now)/1000),label=`out${index}`;filters.push(`[${index}:a]afade=t=out:st=0:d=${remaining.toFixed(3)}[${label}]`);labels.push(`[${label}]`)}else{const label=`current${index}`,remaining=new Date(input.item.endAt).getTime()-now,fade=input.item.kind==="stream"&&input.item.timingType==="flexible"&&remaining<3000?`afade=t=out:st=0:d=${Math.max(.1,remaining/1000).toFixed(3)}`:"anull";filters.push(`[${index}:a]${fade}[${label}]`);labels.push(`[${label}]`)}});filters.push(`${labels.join("")}amix=inputs=${inputs.length}:duration=longest:dropout_transition=0:normalize=0,alimiter=limit=0.95[mix]`);args.push("-filter_complex",filters.join(";"),"-map","[mix]")}
  else if(status.current.kind==="stream"&&status.current.timingType==="flexible"&&currentRemaining>3)args.push("-af",`afade=t=out:st=${Math.max(0,currentRemaining-3).toFixed(3)}:d=3`);
  ...
  decoder.stdout.on("data",chunk=>{lastPcm=Date.now();if(encoder?.stdin.writable)encoder.stdin.write(chunk)});decoder.stderr.on("data",data=>process.stderr.write(data));decoder.on("exit",code=>{if(decoder===thisDecoder){log(`Source pipeline ended (${code})`);decoder=null;if(activeSignature===thisSignature)activeSignature=""}});
}
