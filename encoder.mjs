...
async function startDecoder(status,wanted){
  ...
  try{inputs=await Promise.all(inputs.map(async input=>input.item.kind==="upload"?{...input,playbackUrl:await cacheUpload(input.item,input.playbackUrl)}:input));if(stopping||pendingSignature!==wanted)return;stopDecoder();
  ...
  activeSignature=wanted;log(inputs.length>1?`Mixing ${(status.overlays||[]).map(item=>item.title).join(", ")} under ${status.current.title}`:`Starting ${status.current.title} (${status.current.id})`);const thisSignature=activeSignature;
...
async function scheduleLoop(){while(!stopping){try{const status=await readStatus(),wanted=signature(status);if(status.next?.kind==="upload")void cacheUpload(status.next,`/api/media/${status.next.assetId}`).catch(error=>log(`Couldn’t pre-cache ${status.next.title}: ${error.message}`));if(wanted!==activeSignature&&wanted!==pendingSignature)void startDecoder(status,wanted)}catch(error){log(error.message)}await new Promise(resolve=>setTimeout(resolve,250))}}
