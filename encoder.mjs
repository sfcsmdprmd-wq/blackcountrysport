...
let encoder = null, decoder = null, fetcher = null, activeId = null, lastPcm = 0, stopping = false;
...
function stopDecoder(){
  if (fetcher){ fetcher.kill("SIGTERM"); fetcher=null; }
...
function startDecoder(item, playbackUrl){
  ...
  let input = new URL(playbackUrl, site).toString();const overrideHost=env.STREAM_HOST_OVERRIDE_HOST?.trim(),overrideIp=env.STREAM_HOST_OVERRIDE_IP?.trim(),source=item.kind==="stream"&&item.sourceUrl?new URL(item.sourceUrl):null,useOverride=!!source&&!!overrideHost&&!!overrideIp&&source.hostname===overrideHost;
  if(useOverride)input="pipe:0";
  const remaining=Math.max(1,(new Date(item.endAt).getTime()-Date.now())/1000), elapsed=Math.max(0,(Date.now()-new Date(item.startAt).getTime())/1000);
  ...
  decoder=spawn("ffmpeg",args,{stdio:[useOverride?"pipe":"ignore","pipe","pipe"]});
  if(useOverride&&source){const port=source.port||("https:"===source.protocol?"443":"80");log(`Using DNS override ${overrideHost} -> ${overrideIp}`);fetcher=spawn("curl",["--fail","--silent","--show-error","--location","--no-buffer","--resolve",`${overrideHost}:${port}:${overrideIp}`,source.toString()],{stdio:["ignore","pipe","pipe"]});fetcher.stdout.pipe(decoder.stdin);fetcher.stderr.on("data",d=>process.stderr.write(d));fetcher.on("exit",()=>{fetcher=null;decoder?.stdin.end()});decoder.stdin.on("error",()=>{})}
